...
export default function EditModelInput() {
  ...
  useEffect(() => {
    ...
    const fetchModelInput = async () => {
      try {
        const response = await axios.get(`/api/v1/model-inputs/${id}`)
        const data = response.data
        setFormData({
          user_id: data.user_id || '',
          model_id: data.model_id || '',
          description: data.description || '',
          model_inputs: data.model_inputs || formData.model_inputs,
          hydraulic_conductivity: data.hydraulic_conductivity || formData.hydraulic_conductivity
        })
      } catch (e: any) {
        setError(e?.response?.data?.detail || e?.message || 'Failed to load model input')
      }
    }
      ...
      <div style={{ 
        ...
        <div style={{ textAlign: 'center' }}>
          <h3 style={{ color: 'var(--text-muted)', margin: '0 0 1rem 0' }}>
            Please log in to edit model inputs
          </h3>
          <p style={{ color: 'var(--text-disabled)', margin: 0 }}>
            Authentication required to edit model inputs
          </p>
        </div>
      </div>
    )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`/api/v1/model-inputs/${id}`, formData)
      navigate('/model-inputs')
    } catch (e: any) {
      setError(e?.response?.data?.detail || e?.message || 'Failed to update model input')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div>
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ 
          ...
          Update aquifer model parameters and hydraulic conductivity
        </p>
      </div>
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            ...
            >
              {loading ? 'Updating...' : 'Update Model Input'}
            </button>
